...
// ========================================
// Arrival Validation (Direct Allocation form)
// ========================================
...
function populateDriverSelect(drivers, availabilityLabel) {
  ...
  if (drivers.length > 0) {
    ...
    var stillAvailable = drivers.some(function (d) {
      return String(d.driver_id) === String(previouslySelected);
    });
    ...
    if (infoText) {
      infoText.innerHTML =
        '<i class="fas fa-exclamation-circle" style="color:#c62828;"></i> No drivers available';
...
function filterAvailableDrivers() {
  ...
  if (isRange) {
    var infoText = document.getElementById("driverAvailabilityInfo");
    var drivers =
      typeof ALL_DRIVERS_WITH_CAR !== "undefined" ? ALL_DRIVERS_WITH_CAR : [];
    if (infoText) {
      infoText.innerHTML =
        '<i class="fas fa-info-circle" style="color:#1a237e;"></i> Showing all drivers — conflicting dates will be skipped automatically';
  ...
  if (!date || !pickupTime || !dropoffTime) {
    if (infoText) {
      infoText.innerHTML =
        '<i class="fas fa-info-circle"></i> Select date and time';
  ...
  fetch(
    "/admin/api/available_drivers.php?date=" +
      ...
      "&dropoff=" +
      encodeURIComponent(dropoffTime),
  )
    ...
    .then(function (data) {
      if (data.success) {
        populateDriverSelect(
          data.data,
          data.data.length + " driver(s) available",
        );
      }
    })
    .catch(function (error) {
      ...
      if (infoText) {
        infoText.innerHTML =
          '<i class="fas fa-exclamation-circle" style="color:#c62828;"></i> Error loading drivers';
        infoText.style.color = "#c62828";
      }
    });
}
...
// ========================================
// Arrival Validation (Edit Trip modal)
// ========================================
...
function deleteUnusedPassengers() {
  if (
    !confirm(
      "This will permanently delete every passenger with no trips assigned. Continue?",
    )
  )
    return;
      ...
      if (data.success) {
        if (data.deleted_count > 0 && passengerChoicesInstance) {
          ...
          var remaining = Array.from(select.options)
            .filter(function (o) {
              return (
                data.deleted_ids.indexOf(o.value) === -1 &&
                data.deleted_ids.indexOf(parseInt(o.value)) === -1
              );
            })
            .map(function (o) {
              return {
        ...
        if (messageDiv)
          messageDiv.innerHTML =
            ...
            (data.message || "Error cleaning up passengers.") +
            "</span>";
      }
    })
    .catch(function () {
      if (messageDiv)
        messageDiv.innerHTML =
          '<span style="color: #c62828;">Error cleaning up passengers. Please try again.</span>';
    });
}
...
function updatePassengerDisplay() {
  ...
  if (!select) return;
  var selectedCount = select.selectedOptions
...
function addPassenger() {
  ...
  if (!name) {
    if (messageDiv)
      messageDiv.innerHTML =
        '<span style="color: #c62828;">Please enter a passenger name.</span>';
    return;
  }
  ...
  if (duplicate) {
    if (messageDiv)
      messageDiv.innerHTML =
        '<span style="color: #f57c00;">Passenger already exists in the list.</span>';
    return;
  }
  ...
  fetch(window.location.href, { method: "POST", body: formData })
    ...
    .then(function (data) {
      if (data.success) {
        ...
        if (messageDiv)
          messageDiv.innerHTML =
            '<span style="color: #c62828;">' + data.message + "</span>";
      }
    });
}

function deletePassenger(passengerId, passengerName) {
  if (!confirm('Are you sure you want to delete "' + passengerName + '"?'))
    return;
  ...
  fetch(window.location.href, { method: "POST", body: formData })
      ...
      if (data.success) {
        if (passengerChoicesInstance) {
          var remaining = Array.from(
            document.getElementById("passengerGrid").options,
          )
            .filter(function (o) {
              return String(o.value) !== String(passengerId);
            })
            .map(function (o) {
              return {
                value: o.value,
    ...
    .catch(function () {
      if (messageDiv)
        messageDiv.innerHTML =
          '<span style="color: #c62828;">Error deleting passenger. Please try again.</span>';
    });
}
...
function validatePassengerSelection() {
  ...
  var passengerError = document.getElementById("passengerError");
  var selectedCount =
    select && select.selectedOptions ? select.selectedOptions.length : 0;
...
function openReviewModal(
  ...
  try {
    _reviewDrivers = JSON.parse(driversJson || "[]");
  } catch (e) {
    _reviewDrivers = [];
  }
  ...
  document.getElementById("approveDropoffTime").textContent =
  ...
  document.getElementById("approveTravelType").textContent =
    travelType || "Not specified";
  document.getElementById("approvePurpose").textContent =
  ...
  if (passengers.length > 0) {
    ...
    passengersHtml =
      '<span style="color:#6c757d; font-size:0.85rem;">None</span>';
  }
    ...
    if (codingDay.toLowerCase() === dayOfWeek.toLowerCase()) {
      ...
      openCodingWarningModal(carData, _reviewTripDateRaw, function () {
        showSubmittingOverlay(
          "Approving request and sending confirmation email...",
        );
...
function openEditModal(tripId) {
  ...
  fetch(
    ...
    .catch(function () {
      body.innerHTML =
        '<div class="alert alert-error">Error loading trip details. Please try again.</div>';
    });
}
  ...
  var driverOptions = drivers
    .map(function (d) {
      ...
      return (
        ...
        d.driver_id +
        '" ' +
        (d.is_current ? "selected" : "") +
        ' data-car-id="' +
        (d.car_id || "") +
        '" data-brand="' +
        escapeHtml(d.brand || "") +
        '" data-plate="' +
        escapeHtml(d.plate_number || "") +
        '" data-parking="' +
        escapeHtml(d.parking || "") +
        '">' +
        escapeHtml(d.name) +
        label +
        (d.brand
        ...
        <form id="editApprovedForm">
            ...
            <div class="form-row-2">
                ...
                <div class="floating-group">
                    ...
                    <label for="edit_approved_pickup_time">Departure<span class="required">*</span></label>
                </div>
            </div>
            <div class="form-row-2">
                <div class="floating-group">
                    ...
                    <label for="edit_approved_dropoff_time">Arrival<span class="required">*</span></label>
        ...
        .then(function (data) {
          body.innerHTML = data.success
            ? '<div class="alert alert-success">' +
              data.message +
              '</div><div style="margin-top:15px;"><button type="button" class="btn btn-primary" onclick="closeEditModal(); refreshOpenApprovedDriverModal();">OK</button></div>'
            : '<div class="alert alert-error">' + data.message + "</div>";
        })
        .catch(function () {
          body.innerHTML =
            '<div class="alert alert-error">Error updating trip. Please try again.</div>';
        });
    });
  }
}
...
function openEditPendingModal(tripId) {
  ...
  fetch(
    ...
    .catch(function () {
      body.innerHTML =
        '<div class="alert alert-error">Error loading request details. Please try again.</div>';
    });
}
        ...
        <form id="editPendingForm">
            ...
            <div class="form-row-2">
                ...
                <div class="floating-group">
                    ...
                    <label for="edit_pending_pickup_time">Departure<span class="required">*</span></label>
                </div>
            </div>
            <div class="floating-group">
                ...
                <label for="edit_pending_dropoff_time">Arrival<span class="required">*</span></label>
      ...
      fetch(window.location.pathname, {
        ...
        .then(function (data) {
          body.innerHTML = data.success
            ? '<div class="alert alert-success">' +
              data.message +
              '</div><div style="margin-top:15px;"><button type="button" class="btn btn-primary" onclick="closeEditPendingModal(); location.reload();">OK</button></div>'
            : '<div class="alert alert-error">' + data.message + "</div>";
        })
        .catch(function () {
          body.innerHTML =
            '<div class="alert alert-error">Error updating request. Please try again.</div>';
        });
    });
  }
}
...
document.addEventListener("click", function (e) {
  ...
  var completeInprogressModal = document.getElementById(
    "completeInprogressModal",
  );
  ...
  if (completeInprogressModal && e.target === completeInprogressModal)
    closeCompleteInprogressModal();
  if (cancelModal && e.target === cancelModal) closeCancelModal();
  if (codingWarningModal && e.target === codingWarningModal)
    closeCodingWarningModal();
  if (editModal && e.target === editModal) closeEditModal();
  if (editPendingModal && e.target === editPendingModal)
    closeEditPendingModal();
  if (approvedDriverModal && e.target === approvedDriverModal)
    closeApprovedDriverModal();
  if (outgoingDriverModal && e.target === outgoingDriverModal)
...
function openConfirmModal(formData) {
  ...
  document.getElementById("confirmPickupTime").textContent =
    formData.pickup_time ? formatTimeDisplay(formData.pickup_time) : "-";
  document.getElementById("confirmDropoffTime").textContent =
    formData.dropoff_time ? formatTimeDisplay(formData.dropoff_time) : "-";
  document.getElementById("confirmDriver").textContent =
    formData.driver_name || "-";
  document.getElementById("confirmCar").textContent = formData.car_brand
    ? formData.car_brand + " (" + formData.car_plate + ")"
    : "-";
  document.getElementById("confirmParking").textContent =
    formData.car_parking || "Not specified";
  document.getElementById("confirmCoding").textContent =
    formData.coding_day || "None";
  document.getElementById("confirmPickupLocation").textContent =
    formData.pickup_location || "-";
  document.getElementById("confirmDropoffLocation").textContent =
  ...
  if (formData.passengers && formData.passengers.length > 0) {
    formData.passengers.forEach(function (p) {
      passengersHtml +=
        '<span class="passenger-tag-mini">' + escapeHtml(p) + "</span>";
    });
  } else {
    passengersHtml =
      '<span style="color:#6c757d; font-size:0.8rem;">None</span>';
  }
...
document
  ...
  .addEventListener("submit", function (e) {
    ...
    var selectedPassengerOptions = passengerSelect
      ? Array.from(passengerSelect.selectedOptions)
      : [];
      ...
      checkRangeConflicts(
        ...
        function (conflicts) {
          if (conflicts.length > 0) {
            ...
            document
              .getElementById("rangeConflictModal")
              .classList.add("active");
            document.body.style.overflow = "hidden";
            document.getElementById("rangeConflictProceedBtn").onclick =
              function () {
    ...
    function showDirectAllocationConfirm() {
      ...
      var passengerNames = passengerSelect
        ? Array.from(passengerSelect.selectedOptions).map(function (o) {
...
document
  .getElementById("codingProceedBtn")
  .addEventListener("click", function () {
...
function openDeleteModal(tripId, type, carBrand, tripDate) {
  ...
  document.getElementById("deleteModalMessage").textContent =
    "Are you sure you want to delete this approved trip?";
  document.getElementById("confirmDeleteBtn").href =
    "?delete_approved=" + tripId + "&tab=approved";
...
// ========================================
function openCompleteInprogressModal(
  tripId,
  carBrand,
  driverName,
  tripDate,
...
document
  ...
  .addEventListener("click", function (e) {
    ...
    if (
      href.indexOf("cancel_approved") !== -1 ||
      href.indexOf("cancel_inprogress") !== -1
    ) {
      showSubmittingOverlay(
        "Cancelling trip and sending notification email...",
      );
    }
  });
...
function refreshOpenApprovedDriverModal() {
  ...
  fetch(
    window.location.pathname +
      "?ajax=1&tab=approved&format=json&approved_date=" +
      encodeURIComponent(date) +
      "&t=" +
      Date.now(),
  )
    ...
    .then(function (drivers) {
      var match = drivers.find(function (d) {
        return String(d.driver_id) === String(driverId);
      });
      if (match) {
...
function openApprovedDriverModal(data) {
  ...
  document.getElementById("approvedDriverModalTitle").textContent =
    data.driver_name + " — Approved Trips";

  var subtitleParts = [];
  subtitleParts.push(
    '<span class="driver-modal-count">' +
      data.trips.length +
      " trip" +
  ...
  subtitleParts.push(
    '<span style="color:#343434; font-weight:700;">' +
      escapeHtml(data.car_brand) +
      " (" +
      escapeHtml(data.car_plate) +
      ")</span>",
  );
  if (data.driver_mobile)
    subtitleParts.push(
      '<span style="color:#2e7d32; font-weight:700;">' +
        escapeHtml(data.driver_mobile) +
        "</span>",
    );
  document.getElementById("approvedDriverModalSubtitle").innerHTML =
  ...
  data.trips.forEach(function (t) {
    ...
    tr.appendChild(
      makeCell(
        escapeHtml(t.requestor) +
          '<br><span class="text-muted" style="font-size:0.65rem;">' +
          escapeHtml(t.requestor_email || "") +
          "</span>",
      ),
    );
    ...
    dateCell.innerHTML =
      ...
      (t.dropoff_time ? " – " + formatTimeDisplay(t.dropoff_time) : "") +
      "</span>";
    ...
    tr.appendChild(
      makeCell(
        escapeHtml(t.pickup_location) +
          '<span style="color:#adb5bd;"> → </span>' +
          escapeHtml(t.dropoff_location || "-"),
      ),
    );
    ...
    var passengersHtml =
      t.passengers && t.passengers.length > 0
        ? t.passengers
            .map(function (p) {
              return (
                '<span class="passenger-tag" style="font-size:0.65rem;">' +
                escapeHtml(p.passenger_name) +
                "</span>"
              );
            })
    ...
    if (remarksHtml !== "-") {
      ...
      if (purpose || travelType) {
        ...
        if (travelType)
          remarksHtml +=
            '<span style="font-size:0.6rem; color:#6c757d; text-transform:uppercase; letter-spacing:0.3px;">' +
            escapeHtml(travelType) +
            "</span><br>";
        if (purpose)
          remarksHtml +=
    ...
    if (canStart) {
      ...
      startBtn.title = t.startability
        ? t.startability.reason
        : "Cannot start this trip right now.";
    }
...
function closeApprovedDriverModal() {
  ...
  var dtElements = modal.querySelectorAll(
...
function openOutgoingDriverModal(data) {
  ...
  document.getElementById("outgoingDriverModalTitle").textContent =
    data.driver_name + " — Trip Details";
  ...
  document.getElementById("outgoingDriverModalSubtitle").innerHTML =
    subtitleParts.join(" &middot; ");
  ...
  data.trips.forEach(function (t) {
    ...
    if (
      t.status === "completed" &&
      (t.actual_pickup_time || t.actual_dropoff_time)
    ) {
      dateTimeHtml +=
        '<br><span style="font-size:0.65rem; color:#2e7d32;">Actual: ' +
        (t.actual_pickup_time ? formatTimeDisplay(t.actual_pickup_time) : "-") +
        " – " +
        (t.actual_dropoff_time
          ? formatTimeDisplay(t.actual_dropoff_time)
          : "-") +
        "</span>";
    }
    ...
    tr.appendChild(
      makeCell(
        '<span class="badge ' +
          getStatusBadgeClassJs(t.status) +
          '">' +
          getStatusDisplayJs(t.status) +
          "</span>",
      ),
    );
      ...
      completeBtn.addEventListener("click", function () {
        openCompleteInprogressModal(
          ...
          t.actual_pickup_time
            ? formatTimeDisplay(t.actual_pickup_time)
            : formatTimeDisplay(t.pickup_time) + " (scheduled)",
        );
      });
...
function initApprovedModalTable() {
  ...
  if (dataRows.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="7" style="text-align:center; padding:20px; color:#6c757d;">No approved trips found for this driver.</td></tr>';
    return;
  }
...
function initOutgoingModalTable() {
  ...
  if (dataRows.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="8" style="text-align:center; padding:20px; color:#6c757d;">No trips found for this driver.</td></tr>';
    return;
  }
...
function initPendingTable() {
  ...
  if (!tbody || tbody.children.length === 0) {
    tbody.innerHTML =
      '<tr><td colspan="10" style="text-align:center; padding:20px; color:#6c757d;">No pending requests found.</td></tr>';
  }
...
function openStartTripModal(
  ...
  document.getElementById("startTripCar").textContent =
    carBrand + " (" + carPlate + ")";
...
function showSubmittingOverlay(subtitle) {
  ...
  if (subtitleEl)
    subtitleEl.textContent = subtitle || "Sending email, please wait.";
...
function refreshApprovedDriverList() {
  ...
  fetch(
    window.location.pathname +
      "?ajax=1&tab=approved&approved_date=" +
      encodeURIComponent(date) +
      "&t=" +
      Date.now(),
  )
...
function refreshOutgoingDriverList() {
  ...
  fetch(
    window.location.pathname +
      "?ajax=1&tab=outgoing&outgoing_date=" +
      encodeURIComponent(date) +
      "&t=" +
      Date.now(),
  )
...
function refreshOpenOutgoingDriverModal() {
  ...
  fetch(
    window.location.pathname +
      "?ajax=1&tab=outgoing&format=json&outgoing_date=" +
      encodeURIComponent(date) +
      "&t=" +
      Date.now(),
  )
